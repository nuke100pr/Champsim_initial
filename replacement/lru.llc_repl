...
pair<uint32_t, uint32_t> CACHE::llc_find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    ...
    {
        uint16_t set_type = manager.get_set_type(set); // get set type
        uint32_t way;
        ...
        if (set_type == COLD || set_type == VERY_COLD) // for cold and very cold sets
        {
            ...
            {
                if (block[set][way].alien == false && block[set][way].valid == false)
                {
            ...
            if (way == NUM_WAY)
            {
                ...
                for (way = 0; way < NUM_WAY; way++)
                {
                    if (block[set][way].alien == false && block[set][way].lru == max_lru)
                    {
        ...
        else // for hot and very hot sets
        {
            ...
            {
                // finding helper set
                int32_t helper = manager.get_helper_set(set);
                ...
                if (helper != -1)
                {
                    ...
                    for (way = 0; way < NUM_WAY; way++)
                    {
                        if (block[helper][way].alien && block[helper][way].valid == false)
                        {
                            DP(if (warmup_complete[cpu]) {
                            cout << "[" << NAME << "] " << __func__ << " instr_id: " << instr_id << " invalid set: " << set << " way: " << way;
                            cout << hex << " address: " << (full_addr>>LOG2_BLOCK_SIZE) << " victim address: " << block[set][way].address << " data: " << block[set][way].data;
                        ...
                        // finding lru victim
                        if (block[helper][way].alien && block[helper][way].lru == max_lru)
                        {
...
void CACHE::llc_update_replacement_state(uint32_t cpu, uint32_t set, uint32_t way, uint64_t full_addr, uint64_t ip, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    ...
    if (warmup_complete[cpu]) // after warmup
    {
        uint16_t set_type = manager.get_set_type(set);

        if (set_type == COLD || set_type == VERY_COLD) // for cold and very cold sets
        {
            if (block[set][way].alien) // if block is alien
            {
                // update lru replacement state of alien blocks in cold set
                for (uint32_t i = 0; i < NUM_WAY; i++)
                {
                    if (block[set][i].alien && block[set][i].lru < block[set][way].lru)
                    {
                        block[set][i].lru++;
                    }
                }

                int32_t parent = manager.get_parent_set(set); // get parent of cold set
                ...
                if (parent != -1)
                {
                    ...
                    for (uint32_t i = 0; i < NUM_WAY; i++)
                    {
                        if (block[parent][i].alien == false && block[parent][i].lru < block[set][way].lru)
                        {
                            block[parent][i].lru++;
                        }
                    }
            ...
            else // if block is not alien
            {
                // update lru replacement state
                for (uint32_t i = 0; i < NUM_WAY; i++)
                {
                    if (block[set][i].alien == false && block[set][i].lru < block[set][way].lru)
                    {
                        block[set][i].lru++;
                    }
                }
        ...
        else // for hot and very hot sets
        {
            ...
            int32_t helper = manager.get_helper_set(set); // get helper set
