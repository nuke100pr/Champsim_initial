...
void CACHE::handle_fill()
{
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
  {
    ...
    {
      manager.update_temperature(set, cpu);
    }
    if (cache_type == IS_LLC)
    {
...
//Change
  if (cache_type == IS_LLC && warmup_complete[cpu]) // for llc
  {
    ...
    if (set_type == COLD || set_type == VERY_COLD) // for cold and very cold sets
    {
      for (uint32_t way = 0; way < NUM_WAY; way++)
      {
        if (block[set][way].alien == false && block[set][way].valid && (block[set][way].tag == packet->address))
        {
    ...
    else // for hot and very hot sets
    {
      ...
      if (match_way == -1)
      {
        ...
        if (helper != -1) // if helper set exists
        {
          for (uint32_t way = 0; way < NUM_WAY; way++)
          {
            if (block[helper][way].alien && block[helper][way].valid && (block[helper][way].tag == packet->address))
            {
